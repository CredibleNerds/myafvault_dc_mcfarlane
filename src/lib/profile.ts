...
export type AvatarKind = "none" | "upload" | "figure";
...
function asKind(value: unknown): AvatarKind {
  return value === "upload" || value === "figure" ? value : "none";
}
  ...
  .handler(async ({ context }): Promise<UserProfile> => {
    ...
    const rows = await sql.query<{
      ...
      `select display_name, bio, location, collector_since, avatar_kind,
              avatar_data, avatar_product_id, favorite_product_ids,
              favorite_lines, favorite_collection_ids, updated_at::text as updated_at
    ...
    return {
      ...
      favoriteCollectionIds: asStringArray(row.favorite_collection_ids).slice(0, 8),
      email,
  ...
  .validator((data: Partial<UserProfile>) => {
    ...
    const year =
      ...
      data.collectorSince <= new Date().getFullYear()
        ? data.collectorSince
        : null;
    const avatarKind = asKind(data.avatarKind);
    let avatarData =
    ...
    if (avatarKind !== "upload") avatarData = null;
    const avatarProductId =
      avatarKind === "figure" && typeof data.avatarProductId === "string"
        ? data.avatarProductId.slice(0, 120)
        : null;
    return {
      displayName,
      bio,
  ...
  .handler(async ({ context, data }): Promise<UserProfile> => {
    ...
    await sql.query(
      `insert into user_profiles (
         ...
         avatar_kind, avatar_data, avatar_product_id,

...
async function buildItems(
  productIds: string[],
  customs: Record<string, CustomShareInput> = {},
): Promise<SharedWishlistItem[]> {
  const unique = [...new Set(productIds.filter((id) => typeof id === "string" && id))];
  if (unique.length > 500) {
    throw new Error("Wishlist is too large to share (max 500 items).");
  }
  const covers = await loadSystemCovers(unique);
  const items: SharedWishlistItem[] = [];
...
export const getMyWishlistShare = createServerFn({ method: "GET" })
  .middleware([authMiddleware])
  .handler(async ({ context }): Promise<MyWishlistShare> => {
    const sql = await getSql();
    try {
...
/**
 * Create or refresh the owner's public wishlist share from current product IDs.
 * Reuses the existing active token when present so the link stays stable.
 */
export const publishWishlistShare = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  ...
  .handler(async ({ context, data }): Promise<MyWishlistShare> => {
    const items = await buildItems(data.productIds, data.customs);
    if (items.length === 0) {
      throw new Error("Add at least one figure to your wishlist before sharing.");
    }

    const sql = await getSql();
    const existing = await sql.query<{ token: string }>(
      `select token from wishlist_shares
       where user_id = $1 and revoked_at is null
       order by updated_at desc
       limit 1`,
      [context.userId],
    ...
    const rows = await sql.query<{
      ...
      `insert into wishlist_shares (token, user_id, title, items, created_at, updated_at, revoked_at)
...
/** Revoke the owner's active wishlist share link(s). */
export const revokeWishlistShare = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
...
export const fetchPublicWishlist = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<PublicWishlist> => {
    const sql = await getSql();
    const rows = await sql.query<{
      title: string;
      items: unknown;
    ...
    if (!row || row.revoked_at) {
      throw new Error("This wishlist link is unavailable.");
    }
